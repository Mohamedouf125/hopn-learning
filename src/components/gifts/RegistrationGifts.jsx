...
const RegistrationGifts = () => {
  ...
  const [choosedGift, setChooosedGift] = useState(null);

  useEffect(() => {
    server
      .post("/courses-api", { level: 1 })
      .then((data) => {
  ...
  const handelChooseCource = () => {
    // server
    //   .post("/choose-courses-login", { "courses[0]": choosedGift })
    //   .then((data) => {
    //     dispatch(oldAccount());
    //   })
    //   .catch((error) => {
            ...
            <div className="flex items-center justify-center gap-10 flex-wrap w-full">
              {gifts.map((gift, index) => {
                return (
                  ...
                  >
                    <div>
                      <img src={gift.image} alt="cv image" className="w-full" />
                    </div>
